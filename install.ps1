# Comprehensive installer for cognis-digital/sbomx (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# sbomx is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "sbomx"
$Url  = "git+https://github.com/cognis-digital/sbomx.git"
$Git  = "https://github.com/cognis-digital/sbomx.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: sbomx"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: sbomx"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: sbomx"; exit 0 }
}
